...
parser.add_argument("-d", "--Death", help="Show the deaths in the Chart. Default is True", action="store_false")
...
with open("time_series_covid19_confirmed_global.csv") as csv_file:
    ...
    for row in csv_reader:
        for t in row[4:len(row)]:
            if row[0] == "":
                cases[row[1].upper()].append(int(t))
            else:
                cases[row[0].upper()].append(int(t))
...
if showDeath:
    with open("time_series_covid19_deaths_global.csv") as csv_file:
        ...
        for row in csv_reader:
            for t in row[4:len(row)]:
                if row[0] == "":
                    deaths[row[1].upper()].append(int(t))
                else:
                    deaths[row[0].upper()].append(int(t))
...
if (showDeath):
    ...
    plt.legend(handles=[blue_patch, blue_line], loc="upper left")
else:
    p2 = axes[0].bar(description[-timeRange:], cases[country][-timeRange:])
    plt.legend(handles=[blue_patch, gray_patch, blue_line], loc="upper left")
if not verbose:
    plt.show()
